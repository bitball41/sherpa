	...
	this.mount = async () => {
		...
		const html = monaco.editor.create(this.htmlbox, {
			...
			<html>
				...
				<body>
					<h1>Sherpa Sandbox Playground</h1>
					<p>
					Sherpa allows any webpage to be run on the same origin in an isolated manner
					</p>
		...
		const recompile = async () => {
			...
			this.frame.src = sherpa.encodeUrl(this.fakeorigin);
		};
