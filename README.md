...
- **Measurably faster.** Sherpa's rewriting pipelines are 1.3–1.8× faster than Scramjet 1.x on the same inputs (up to 5× on inline-script-heavy pages), which translates to 12–26% faster full proxied page loads in Chromium against the published Scramjet 1.1.0 — same service worker pipeline, same transports, same fixture site, engine as the only variable. Methodology, statistics, and a reproducible harness live in [`bench/`](bench/README.md).
