...
- Freshness comes from `Cache-Control: max-age`, then `Expires`, then RFC 9111 heuristic freshness (a tenth of the time since `Last-Modified`, capped at a day); an upstream `Age` is subtracted. It is a _private_ cache — one browser profile, not shared between users — so `private` is storable and `s-maxage` is not consulted. Documents skip heuristic freshness: HTML is often personalized and sent without `Cache-Control`, so a guessed lifetime from `Last-Modified` would replay a logged-in page into a logged-out session. They still store when the origin is explicit (`max-age`/`Expires`) or when a validator lets a `304` skip the rewrite.
...
- **Documents are cacheable.** The cookie jar used to be snapshotted into every rewritten document (`self.COOKIE = …`), so replaying one would boot the client against a stale session. Cookies (and the live configuration) now come from a no-store `${prefix}$boot` script requested as part of the document's parser-blocking boot sequence, so the rewritten HTML itself can be stored under the same rules as any other subresource.
