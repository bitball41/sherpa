...
test("empty referrers and navigation fallback remain explicit", () => {
	const noReferrer = new Request(
		"https://proxy.test/sherpa/target",
	...
	assert.equal(noReferrer.referrer, "");
	assert.equal(navigation.mode, "same-origin");
});

test("stream bodies use duplex without invalid keepalive", () => {
